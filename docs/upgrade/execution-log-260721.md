...
### CP-紧急 · 260721 晚 · 刘野生产实测"四页无法演示" → 逐页真交互诊断 + 五处修复全部上线复验

**教训入账（CC 自我点名）**：此前 CP2 验收面 = spec 断言 + 构建产物 grep，未做逐页真浏览器交互目检——验收面窄于"可演示"宣称面，违反全局"视觉验收看真实渲染"铁律。本节全部结论改为：真浏览器点主操作 + 生产复验取证。

**诊断方法**：Playwright 真浏览器登录生产，逐页点主 CTA，三轮采集（截图 / console / 失败请求 / 全文文本）。

**修复清单（每条均生产复验）**：
1. **credit 演示链形状错配**（`agent_credit/api.py` · 83afa39）：扁平 profile 样本被塞进 Agent6 交接转换器（facts/sections 不存在）→ 公司名/申请额度 500 万/财务锚点全部丢空——A4 拆掉造词兜底后暴露的存量暗病。修：接口层形状分流。生产复验：`amount_provided true×9`、鼎盛综合分 **44**（=封闭基线）、公司名回归、决策「有条件批准」带真实额度
2. **决策留痕全线静默失败**（服务器 DB 缺 `parent_turn_id` 列——5 月 v1.1 迁移从未在生产执行）：所有 agent 的 ledger 写入 `persisted:false`，卖点③名存实亡。修：服务器 `ALTER TABLE`（先备份 `decisions.sqlite.bak-260721`，存量 1467 行）。生产复验：credit 真跑 `persisted:true×2`
3. **riskctrl 通过率 0% + 79% 样本消失**（`no_hit`=未命中任何规则=默认放行，未计入通过桶）：`/backtest`（3649ea0）与 `/demo/run` 内的**复制体聚合**（828458d）同病同修。生产真浏览器复验：**通过 78.8% + 复核 0.7% + 拒绝 20.5% = 100% 闭合**；聚合收单源列入重构
4. **alert 吓人横幅**（`alert_pool_fixture` 模式从未注册进横幅 SSOT 映射 → 掉进"数据来源不明·联系管理员"兜底 · 354e6b7）：补诚实 info 横幅。生产复验：错误横幅消失、信息横幅在位、扫描红 27/黄 24 正常
5. **report 页头旧会话穿帮**（FIX-B1-R3 · f38644d）随 Wave 部署上线

**未修留账（演示绕行 + backlog）**：
- **channel 雷达全同**：结构性——真实网搜证据薄（单信号）+ 评分公式粗桶（30/50/70 档）+ demo 路径相似度全 0；「一键示例」也走真 Tavily（无富数据池）。演示绕行：讲候选清单+证据面板，少停留雷达。backlog：证据基雷达渲染（无证据轴不画假中值）+ `LookAlikeKBMatcher` P3（代码内已留 TODO）
- **compliance 工作台「运行示例扫描」= 政策 vs 制度库冲突比对**，诚实结果仅 1 条 major（且 title=None 显示 bug）——内容太薄；**业务违规巡检（5 严重/1 一般/1 观察）在 chat scenario 路径**，本地引擎复验 5/1/1 全真检出。幕 6 走法（工作台弱内容 vs chat 巡检强内容）待彩排拍板
- riskctrl/alert 页头统计假数（38/0.34/9.4min、3200/4.1% 等）与零位面板 = Stage 3 B8 族清扫范围
- pytest 合跑 `tests/agent_credit tests/upgrade` 会串扰泄漏 Tavily 真调（分开跑全绿）——测试基建怪癖备案，CP 验收统一分开跑
