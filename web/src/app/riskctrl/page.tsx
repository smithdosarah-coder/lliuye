...
export default function RiskctrlPage() {
      ...
      <header className="flex items-start justify-between mb-8 pb-6 border-b border-[var(--color-ink)]">
        <div>
          <div className="text-[11px] font-tabular tracking-[0.3em] text-[var(--color-brass)] uppercase">
            A02 · Strategy Operations
          </div>
          <h1 className="mt-2 font-display text-[36px] leading-tight text-[var(--color-ink)]">
            风控策略运营
          </h1>
          <p className="mt-1 text-[13px] text-[var(--color-ink-muted)] max-w-[720px]">
            用历史样本回测新策略：业务上是否更赚钱？坏账是否受控？给出可直接拍板的上线建议。
            <span className="inline-block mt-1">
              <span className="text-[var(--color-brass)]">输入 </span>
              基线策略 · 候选策略 · 回测样本 →
              <span className="text-[var(--color-brass)]"> 产出 </span>
              上线建议 + 业务影响 + 技术指标 + 规则对照 + 混淆矩阵
            </span>
          </p>
        </div>
