...
工具域（MCP 式拆分）：
  ...
  - 评分计算域：对公 / 对私双模型可切换，输出四维分数
      入口：scoring_model_corporate / scoring_model_retail
  - 红线检查域：规则引擎 v2，确定性判定不让 LLM 现场算
      入口：rule_engine_v2 / risk_appetite_config
  - 案例召回域：历史相似案例检索 + 同业对标
      入口：case_retriever
  - 决策编排域：组装输出 Dashboard，生成审贷建议文案
      入口：decision_engine / advisor_formatter
...
工具域公开 API（`<域>_<动作>` 命名）都在 `agent_credit.domains` 子包下：
  `from agent_credit.domains import scoring_calc_corporate, redline_check_rules_v2, ...`
"""
