# -*- coding: utf-8 -*-
"""Agent6 · QC Blocker 四维强化(Phase 2 Task B)

职责:在 LLM 章节生成与 Phase 3 自审完成后,对最终文本做四维硬质量闸门:
  1. placeholder    占位符残留(企业名/数字/模板指导文字/区间数字/补全提示词)
  2. evidence       证据链完整性(关键数字/主体名/结论是否挂出处)
  3. financial_consistency  财务数字与 financial_anchor 一致(允许 ±1% 容差)
...
def run_blocker(
    ...
    summary = (
        ...
        if blocked
        else f"PASS · 通过 4 维 QC Blocker(警告 {len(issues)} 条)"
    )
