...
> **知识仓库引用（个人库 second-brain）· 全局知识路由 SOP 本仓强化**
> 本仓 `docs/` = **工程事实源**（架构 / 接口 / 部署 / 决策）优先查。
> 跨项目的**产品判断 / PRD / 复盘结论 / 历史上下文 / 个人 IP / 教学** → 查个人库 `D:/second-brain`：先 `整理/hot.md`（~500 字）→ `整理/系统区/_自动清单.md` → `整理/index.md` → 具体页。信贷 6Agent 的 PRD / 复盘 / 产品树 / 决策追溯都在库里。
> 通用编程 / 本仓 docs 已够 → **不查**（省 token）。全局规则见 `~/.claude/CLAUDE.md` 知识路由 SOP。
