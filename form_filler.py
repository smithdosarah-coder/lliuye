...
class FormFillAgent:
    ...
    def run(self, template_path: str, output_path: str,
        ...
        # ═══════════════════════════════════════════════════════════
        use_section_generation = True  # 可配置开关
        _section_gen_success = False

        if use_section_generation:
            try:
                self._log(progress_cb, "★ 启动逐节生成模式（替代逐字段填写）...")

                # Step 1: 模板拆解 — 分类每个段落为 SKELETON / CONTENT
                decomposition = decompose_template(doc)
                body_sections = decomposition["body_sections"]
                body_paragraphs = decomposition["body_paragraphs"]
                stats = decomposition["stats"]

                self._log(progress_cb,
                          f"  模板拆解完成: {stats['section_count']}个章节, "
                          f"{stats['skeleton_count']}个骨架段, "
                          f"{stats['content_count']}个待重写段")

                sections_to_rewrite = [s for s in body_sections if s.has_content_to_rewrite]
                if not sections_to_rewrite:
                    ...
                    section_results = generate_all_sections(
                        ...
                        truth_financial_data=getattr(self, "_truth_financial_data", None),
                    ...
                    if section_results:
                        ...
                        self.stats["filled"] += applied
                        self._log(progress_cb,
                                  f"  逐节生成完成: {applied} 个段落已重写")

                        # Step 4: 输出验证门 — 检测模板泄漏
                        leaks = validate_output(doc, body_paragraphs)
                        if leaks:
                            self._log(progress_cb,
                                      f"  ⚠ 检测到 {len(leaks)} 个疑似模板泄漏段落，"
                                      f"将在 Phase C 中处理")
                        else:
                ...
                self._log(progress_cb, f"  逐节生成出错: {e}，降级到传统模式")
                import traceback
                traceback.print_exc()
                _section_gen_success = False
        ...
        # ═══════════════════════════════════════════════════════════
        # 降级路径：传统语义块/批处理模式
        #   仅在 section generation 失败或未启用时执行
        # ═══════════════════════════════════════════════════════════

        # ★ 改造三: 语义块驱动填写 (可选，根据配置决定是否启用)
        # 将相关字段组织成语义块，一起填写保证一致性
        use_semantic_blocks = not _section_gen_success  # 仅在新模式失败时启用
        semantic_blocks = []
        ...
        # ★ 如果逐节生成成功，跳过 Phase 3（XX字段已在整节重写中处理）
        if fields and not _section_gen_success and not (semantic_blocks and len(semantic_blocks) > 1):
        ...
        # ★ 如果逐节生成成功，跳过（范例段落已在整节重写中处理）
        if examples and _section_gen_success:
            self._log(progress_cb,
                      f"[逐节模式] 跳过 {len(examples)} 个示例段落（已在整节重写中处理）")
        elif examples:
        ...
        # 逐节生成模式已处理主体内容段落，跳过 Phase C 避免重复改写和浪费API调用
        if _section_gen_success:
            self._log(progress_cb,
                      "Phase C: 逐节生成模式已完成主体内容改写，跳过模板残留审查")
        else:
            try:
